  ...
  const login = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      ...
      if (profile?.role === 'docent') {
        window.location.href = '/dashboard/docent'
      } else {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F5F4F1', padding: 16 }}>
      <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E8E6E1', padding: '32px 28px', width: '100%', maxWidth: 420, boxShadow: '0 2px 8px rgba(0,0,0,.06)' }}>
        <div style={{ fontSize: 24, fontWeight: 800, textAlign: 'center', marginBottom: 6 }}>
          Talent<span style={{ color: '#534AB7' }}>Flow</span>
        </div>
        <div style={{ fontSize: 13, color: '#6B6860', textAlign: 'center', marginBottom: 28 }}>
          Inloggen op je stageprofiel
        </div>
        ...
        <form onSubmit={login}>
          <div>
            ...
            <input style={inp} type="email" value={email}
              ...
              placeholder="naam@school.nl" required />
          </div>
          <div>
          ...
          <button style={{ width: '100%', padding: 11, fontSize: 14, fontWeight: 600, background: '#534AB7', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}
